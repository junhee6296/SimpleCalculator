...
- 구현 내용과 기능 설명
    ...
    - isScientificMode = !isScientificMode와 같은 코드로 모드를 반전하고 공학/기본 모드 텍스트를 바꿈
    - 각 계산 기호 버튼마다 공학 모드 여부를 if문으로 판단하여 공학모드의 경우에는 연산자 여러개를 써도 초기화없이 진행시킴
    - 공학모드일때만 if문으로 괄호 기능이 작동하도록 함